...
import { getToolCardCopy, getToolDisplayName } from "@/lib/tools/tool-content";
...
export function DashboardOverview({
  ...
  return (
    <>
            ...
            <div className="mt-6 space-y-3">
              ...
              {gddLocked ? (
                <button
                  ...
                  className="block w-full rounded-2xl border border-[color:var(--border)] bg-[color:var(--background)]/40 p-4 text-left text-sm break-keep leading-relaxed text-[color:var(--foreground)]/75 transition hover:bg-[color:var(--card-strong)]"
                >
      ...
      {gddTool.requiredPlan ? (
        <ToolUpgradeModal
          open={upgradeOpen}
          onClose={() => setUpgradeOpen(false)}
          toolName={gddCopy.title}
          toolDescription={gddCopy.description}
          requiredPlan={gddTool.requiredPlan}
        />
      ) : null}
    </>
  );
