...
import { FileText, Gauge, History, LayoutDashboard, LayoutTemplate, Lightbulb, LogOut, Palette, ScrollText, Settings, Sparkles } from "lucide-react";
...
export function Sidebar({
  ...
  const { t } = useLanguage();
  const [upgradeToolId, setUpgradeToolId] = useState<ToolType | null>(null);
  ...
  const items = [
    ...
    { href: gddTool.route, label: t.dashboard.gdd, icon: FileText, toolId: "gdd" as const, requiredPlan: gddTool.requiredPlan },
  ...
  const selectedTool = upgradeToolId ? getToolConfig(upgradeToolId) : null;
  const selectedToolCopy = upgradeToolId ? getToolCardCopy(upgradeToolId, t) : null;
  ...
  return (
    <>
          ...
          {items.map((item) => {
            ...
            const className = cn(
              ...
              active
                ...
                : locked
                  ? "border border-[color:var(--border)] text-[color:var(--foreground)]/68 hover:bg-[color:var(--card)] hover:text-[color:var(--foreground)]"
            ...
            if (locked && item.toolId) {
              return (
                <button
                  key={item.href}
                  type="button"
                  onClick={() => setUpgradeToolId(item.toolId ?? null)}
      ...
      {selectedTool && selectedToolCopy && selectedTool.requiredPlan ? (
        <ToolUpgradeModal
          open={!!upgradeToolId}
          onClose={() => setUpgradeToolId(null)}
          toolName={selectedToolCopy.title}
          toolDescription={selectedToolCopy.description}
          requiredPlan={selectedTool.requiredPlan}
        />
      ) : null}
    </>
  );
