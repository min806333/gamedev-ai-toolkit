...
export function UsageCard({
  ...
  return (
    <div className="grid gap-6 md:grid-cols-3">
      {cards.map((card) => (
        <Card key={card.label} className="border-[color:var(--border)] bg-[color:var(--card)] p-6">
          <div className="flex items-center justify-between gap-4">
            <p className="text-sm text-[color:var(--foreground)]/45">{card.label}</p>
          ...
          <h2 className="mt-5 text-4xl font-semibold text-[color:var(--foreground)]">{card.value}</h2>
          <p className="mt-2 text-sm text-[color:var(--foreground)]/55">{card.hint}</p>
        </Card>
      ))}
    </div>
  );
