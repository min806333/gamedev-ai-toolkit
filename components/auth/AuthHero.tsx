...
export function AuthHero() {
  ...
  const content = {
    en: {
      ...
      description: "Generate game ideas, Unity scripts, UI layouts, and game design documents instantly.",
      points: ["Prompt-to-prototype workflows", "Production-ready planning outputs", "Built for indie teams and solo devs"]
    },
    ko: {
      eyebrow: "AI 게임 스튜디오",
      title: "AI로 더 빠르게 게임을 만드세요",
      description: "게임 아이디어, Unity 스크립트, UI 레이아웃, 게임 기획 문서를 즉시 생성하세요.",
      points: ["프롬프트에서 프로토타입까지", "실무형 기획 산출물", "인디 팀과 솔로 개발자를 위한 도구"]
    }
  } as const;
    ...
    <section className="relative hidden overflow-hidden md:flex">
      ...
      <div className="relative flex h-full w-full flex-col justify-between p-12 lg:p-16">
        ...
        <div className="max-w-xl">
          ...
          <h1 className="mt-6 max-w-lg text-5xl font-semibold leading-tight text-white">{copy.title}</h1>
          <p className="mt-6 max-w-xl text-lg leading-8 text-white/68">{copy.description}</p>

          <div className="mt-10 grid gap-4 sm:grid-cols-3">
            <div className="rounded-3xl border border-white/10 bg-white/8 p-5 backdrop-blur">
              <Bot className="h-5 w-5 text-cyan-200" />
              <p className="mt-4 text-sm leading-6 text-white/72">{copy.points[0]}</p>
            </div>
            <div className="rounded-3xl border border-white/10 bg-white/8 p-5 backdrop-blur">
              <Layers3 className="h-5 w-5 text-orange-200" />
              <p className="mt-4 text-sm leading-6 text-white/72">{copy.points[1]}</p>
            </div>
            <div className="rounded-3xl border border-white/10 bg-white/8 p-5 backdrop-blur">
              <Gamepad2 className="h-5 w-5 text-emerald-200" />
              <p className="mt-4 text-sm leading-6 text-white/72">{copy.points[2]}</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3 text-sm text-white/45">
          ...
          AI-powered game workflows for concept, code, and production planning
        </div>
      </div>
    </section>
  );
