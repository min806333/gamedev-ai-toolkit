...
export function ResultPanel({
  ...
  return (
    <Card className="min-h-[640px] border-[color:var(--border)] bg-[linear-gradient(180deg,color-mix(in_srgb,var(--card)_94%,white_6%),var(--card))] p-5 sm:p-6 lg:p-7">
      <div className="flex flex-wrap items-start justify-between gap-4 border-b border-[color:var(--border)] pb-5">
        <div className="min-w-0">
          ...
          <p className="mt-2 max-w-xl text-sm break-keep leading-relaxed text-[color:var(--foreground)]/58">{t.common.resultHint}</p>
      ...
      {loading && !result ? (
        <div className="mt-6 flex min-h-[520px] flex-col items-center justify-center gap-5 rounded-2xl border border-[color:var(--border)] bg-[color:var(--background)]/45 px-6 text-center text-sm text-[color:var(--foreground)]/65">
          ...
          <div>
            ...
            <p className="mt-2 break-keep text-[color:var(--foreground)]/50">{t.common.buildingResponse}</p>
          </div>
        </div>
      ) : (
        <div className="mt-6 max-h-[560px] overflow-y-auto rounded-2xl border border-[color:var(--border)] bg-[color:var(--background)]/45 p-5">
          {result ? (
            <div className="prose max-w-none break-keep text-sm leading-7 prose-headings:text-[color:var(--foreground)] prose-p:text-[color:var(--foreground)]/80 prose-li:text-[color:var(--foreground)]/80 prose-strong:text-[color:var(--foreground)] prose-code:text-[color:var(--foreground)] prose-pre:border prose-pre:border-[color:var(--border)] prose-pre:bg-[color:var(--background)]/70">
              <ReactMarkdown>{result}</ReactMarkdown>
            </div>
          ) : (
            <div className="flex min-h-[520px] items-center justify-center rounded-2xl border border-dashed border-[color:var(--border)] px-6 text-center text-sm break-keep text-[color:var(--foreground)]/45">
